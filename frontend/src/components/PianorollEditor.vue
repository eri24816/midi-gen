...
export default {
    ...
    setup(
        props: {
            midiData: ArrayBuffer | Uint8Array | null;
            editable: boolean;
            minPitch: number;
            maxPitch: number;
        },
        ...
        const _play = (): void => {
            ...
            intervalId = setInterval(
                () => {
                    ...
                    piano.update(
                        songStartTime +
                            (cursorPosition - 0.5) / pianoroll.value.bps,
                    );
                    const notesToPlay = pianoroll.value.getNotesOnsetBetween(
                        oldCursorPosition + 0.25,
                        cursorPosition + 0.25,
                    );
                    notesToPlay.forEach((note) => {
                        piano.playNote(
                            songStartTime +
                                (note.onset - 0.125) / pianoroll.value.bps,
                            note.pitch,
                            note.velocity,
                            songStartTime +
                                (note.onset + note.duration - 0.125) /
                                    pianoroll.value.bps,
                        );
                    });
                    render();
